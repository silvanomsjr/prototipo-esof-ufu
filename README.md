...
* [`src/prototipo_esof_ufu/`](src/prototipo_esof_ufu/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/prototipo_esof_ufu/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `prototipo-esof-ufu` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/prototipo_esof_ufu/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns prototipo-esof-ufu.example
  (:require [prototipo-esof-ufu.config :as config])
